...
const ExperienceForm = ({ data, onChange }) => {
    ...
    const generateDescription = async (index) => {
        ...
        try {
            ...
            // server returns { enhanceContent }
            const enhanced = data && (data.enhanceContent || data.enhancedContent)
            if (enhanced) {
           ...
           toast.error(serverMsg || error.message || 'AI enhancement failed')
        } finally {
                ...
                <div key={index} className='p-4 border border-gray-200 rounded-lg space-y-3'>
                    ...
                    <div className="space-y-2">
                        <div className='flex items-center justify-between'>
                            ...
                            <button onClick={()=> generateDescription(index)} disabled={generatingIndex === index || !experience.position || !experience.company} className='flex items-center gap-1 px-2 py-1 text-xs bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors disabled:opacity-50'>
